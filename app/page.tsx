...
export default function Home() {
  ...
  useEffect(() => {
    (async () => {
      ...
      //             홈 뱃지에 계속 잡히는 문제 (백현 케이스)
      //   해결: 아래 3가지 조건 중 하나라도 만족하면 뱃지에서 자동 제외
      //     ① promoted_member_id 가 이미 세팅됨 (members 로 승격 완료)
      //     ② status 가 new/inbox/pending 이외 (대기중/체험/취소 등)
      //     ③ 같은 phone 을 가진 members 가 존재 (동일 인물이 회원 DB에 이미 있음)
      ...
      if (pendingRows && pendingRows.length > 0) {
        ...
        // 자동 자기 정화: 조건 만족 리드는 processed=true 로 백그라운드 UPDATE
        if (autoClearIds.length > 0) {
          console.log(`[v3.40.9] 홈 뱃지 자동 정화: ${autoClearIds.length}건 processed=true 마킹`);
          supabase
            .from("leads_inbox")
            .update({ processed: true, processed_at: new Date().toISOString() })
            .in("id", autoClearIds)
            .then(({ error }) => {
              if (error) console.warn("[v3.40.9] 자기 정화 UPDATE 실패:", error.message);
            });
        }
      }

      console.log(`[v3.40.9] 홈 뱃지 계산: 진짜 미처리 신규=${realPending}건 (자동 정화 ${autoClearIds.length}건)`);
      // ✅ v3.24.2: 회원 DB 배지 제거 - 상담 리드 신규 유입 시에만 알림 표시
      setBadges({ inbox_pending: realPending });
    })();
  }, []);
