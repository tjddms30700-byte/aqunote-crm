...
/**
 * ═══════════════════════════════════════════════════════════════
 * 🔀 v3.45.5 - Kakao 통합 라우터
 * ═══════════════════════════════════════════════════════════════
 * POST /api/kakao?action=parse   → 파일 파싱 (미리보기)
 * POST /api/kakao?action=import  → 파싱 결과 DB 저장
 * ═══════════════════════════════════════════════════════════════
 */
...
async function handleKakaoParse(req: Request) {
  try {
...
async function handleKakaoImport(req: Request) {
  try {
    // 1) 요청 파싱
    let body: any;
    try {
...
export async function POST(req: NextRequest) {
  ...
  switch (action) {
    case "parse":  return handleKakaoParse(req);
    case "import": return handleKakaoImport(req);
    default:
      return NextResponse.json(
        { error: `Unknown action: ${action}. Use ?action=parse|import` },
        { status: 400 }
      );
  }
}
