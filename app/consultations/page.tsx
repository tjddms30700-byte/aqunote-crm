...
export default function ConsultationsPage() {
  ...
  // slot_matrix에서 해당 셀을 찾음
  function getMatrixCell(dayIdx: number, timeSlot: string) {
    return matrix.find(m => m.day_of_week === dayIdx && m.time_slot === timeSlot);
  }

  // 셀 상태 업데이트 (관리자 수동 편집)
  async function updateMatrixCell(dayIdx: number, timeSlot: string, patch: any) {
    const existing = getMatrixCell(dayIdx, timeSlot);
    if (existing) {
      const { error } = await supabase.from("slot_matrix")
        .update({ ...patch, updated_at: new Date().toISOString() })
        .eq("id", existing.id);
      if (error) { alert("수정 실패: " + error.message + "\n\n💡 AQUNOTE_V38_SLOTMATRIX.sql을 먼저 실행해주세요."); return; }
    } else {
      const orgId = (await supabase.from("organizations").select("id").limit(1).single()).data?.id;
      const { error } = await supabase.from("slot_matrix").insert({
        org_id: orgId, day_of_week: dayIdx, time_slot: timeSlot, ...patch,
      });
  ...
  // slot_matrix 기반 상태 판단 (관리자가 수동 설정)
  function slotStatus(dayIdx: number, timeSlot: string): "open" | "fixed" | "closed" {
    const cell = getMatrixCell(dayIdx, timeSlot);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-aqu-50 to-white p-6">
      ...
      <div className="max-w-7xl mx-auto mb-4 flex gap-2">
        ...
        {/* 자체 신청폼 사용 - AQU LAB Google Sheets 연동 제거됨 */}
        <Link
          href="/inbox"
          className="ml-auto px-3 py-2 text-xs bg-purple-50 border border-purple-200 text-purple-700 rounded-lg hover:bg-purple-100 flex items-center gap-1"
        >
          📥 신규 유입 관리
        </Link>
      </div>
        ...
        <div className="max-w-7xl mx-auto bg-white rounded-xl border border-aqu-100 overflow-x-auto">
          <div className="p-4 border-b border-aqu-100 bg-gradient-to-r from-emerald-50 to-teal-50">
            ...
            <p className="text-xs text-emerald-700 mt-1">
              셀을 클릭해 상태 변경: 🟢 빈자리 (대기자 자동 매칭) · 🔵 고정수업 (회원명 입력) · ⬛ 운영 안함
            </p>
          </div>
          <table className="w-full text-xs">
              ...
              {TIME_SLOTS.map((t) => (
                <tr key={t}>
                  ...
                  {DAYS.slice(1).map((d, i) => {
                    ...
                    const cell = getMatrixCell(dayIdx, t);
                    ...
                    if (st === "closed") {
                      return (
                        <td key={d}
                          ...
                          className="p-1 border border-emerald-100 bg-gray-100 text-center text-gray-400 cursor-pointer hover:bg-gray-200"
                          title="클릭하여 상태 변경">⬛ 운영 안함</td>
                      );
                    }
                    // 고정수업
                    if (st === "fixed") {
                      return (
                        <td key={d}
                          onClick={() => setEditCell({ dayIdx, time: t, row: cell })}
                          className="p-1 border border-emerald-100 bg-blue-50 text-center cursor-pointer hover:bg-blue-100"
                          title="클릭하여 수정">
                          <div className="font-bold text-blue-700">🔵 고정</div>
                          <div className="text-[11px] text-blue-800 truncate">{cell?.fixed_name || "-"}</div>
                        </td>
                      );
                    }
                    // OPEN (빈자리) - 대기자 자동 매칭
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl p-6 w-full max-w-md" onClick={e => e.stopPropagation()}>
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-lg font-bold text-aqu-900">🎯 시간대 상태 설정</h3>
          <button onClick={onClose}><X className="w-5 h-5" /></button>
        </div>

        <div className="mb-4 p-3 bg-emerald-50 rounded-lg text-sm">
          <div className="font-medium text-emerald-900">{dayLabel}요일 · {time}</div>
        </div>

        <div className="space-y-3">
          <div>
            <label className="text-sm font-medium text-gray-700 mb-2 block">상태 선택</label>
            <div className="grid grid-cols-3 gap-2">
              <button onClick={() => setStatus("open")}
                className={`p-3 rounded-lg border-2 text-sm ${status === "open" ? "border-emerald-500 bg-emerald-50 text-emerald-700 font-bold" : "border-gray-200 bg-white text-gray-600"}`}>
                🟢<br/>빈자리
              </button>
              <button onClick={() => setStatus("fixed")}
                className={`p-3 rounded-lg border-2 text-sm ${status === "fixed" ? "border-blue-500 bg-blue-50 text-blue-700 font-bold" : "border-gray-200 bg-white text-gray-600"}`}>
                🔵<br/>고정수업
              </button>
              <button onClick={() => setStatus("closed")}
                className={`p-3 rounded-lg border-2 text-sm ${status === "closed" ? "border-gray-500 bg-gray-100 text-gray-700 font-bold" : "border-gray-200 bg-white text-gray-600"}`}>
                ⬛<br/>운영 안함
              </button>
            </div>
          </div>
          ...
          {status === "open" && (
            <div className="p-3 bg-emerald-50 border border-emerald-200 rounded-lg text-xs text-emerald-700">
              💡 <b>빈자리</b>로 지정하면 이 시간대를 희망하는 <b>대기중인 회원</b>이 자동으로 이 셀에 표시됩니다.
            </div>
          )}
        </div>
        ...
        <div className="flex gap-2 mt-6">
          ...
          <button onClick={() => onSave({ status, fixed_name: status === "fixed" ? fixedName : null, note })}
            disabled={status === "fixed" && !fixedName.trim()}
