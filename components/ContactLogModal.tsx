...
export default function ContactLogModal({ member, onClose, onSaved }: any) {
  ...
  // ✅ v3.36.1: lead_ 접두어 분기 - 상담 리드와 정회원을 적절한 테이블로 라웅팅
  const rawId = String(member?.id || "");
  const isLead = rawId.startsWith("lead_");
  const pureId = isLead ? rawId.replace("lead_", "") : rawId;
  ...
  async function loadLogs() {
    if (!pureId) return;
    setLoading(true);
    // ✅ v3.36.1: 리드인 경우 leads.raw_payload.contact_logs, 정회원은 contact_logs 테이블
    if (isLead) {
      const { data: leadData } = await supabase.from("leads").select("raw_payload").eq("id", pureId).maybeSingle();
      const extraLogs = leadData?.raw_payload?.contact_logs || [];
      setLogs(Array.isArray(extraLogs) ? extraLogs : []);
    } else {
      // 1) contact_logs 테이블 시도
      const { data, error } = await supabase.from("contact_logs")
        .select("*").eq("member_id", pureId).order("contact_at", { ascending: false });
      if (!error && data) {
        setLogs(data);
      } else {
  ...
  async function save() {
    ...
    // ✅ v3.36.1: lead_ 접두어가 있으면 leads.raw_payload.contact_logs에 저장
    if (isLead) {
      const { data: leadData, error: leadErr } = await supabase.from("leads").select("raw_payload").eq("id", pureId).maybeSingle();
      if (leadErr) {
        setSaving(false);
        return alert("리드 조회 실패: " + leadErr.message);
      }
      const prev = Array.isArray(leadData?.raw_payload?.contact_logs) ? leadData.raw_payload.contact_logs : [];
      const newPayload = {
        ...(leadData?.raw_payload || {}),
        contact_logs: [record, ...prev].slice(0, 100),
      };
      const { error: ue } = await supabase.from("leads").update({ raw_payload: newPayload }).eq("id", pureId);
      if (ue) {
        setSaving(false);
        return alert("저장 실패: " + ue.message);
      }
      console.log(`[v3.36.1] ✅ 리드 응대기록 저장: leads.raw_payload id=${pureId}`);
    } else {
      // 정회원: contact_logs 테이블 INSERT (member_id 순수 UUID)
      const insertRecord = { ...record, member_id: pureId };
      const { error } = await supabase.from("contact_logs").insert(insertRecord);

      if (error) {
        // fallback: members.extra.contact_logs
        const prev = Array.isArray(member?.extra?.contact_logs) ? member.extra.contact_logs : [];
        const newExtra = {
          ...(member.extra || {}),
          contact_logs: [record, ...prev].slice(0, 100),
        };
        const { error: ue } = await supabase.from("members").update({ extra: newExtra }).eq("id", pureId);
        if (ue) {
          setSaving(false);
          return alert("저장 실패: " + ue.message);
        }
      }
      console.log(`[v3.36.1] ✅ 정회원 응대기록 저장: id=${pureId}`);
    }
